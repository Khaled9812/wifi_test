...
{
    /* Enable the IOMUXC clock so we can program the pads. */
    CLOCK_EnableClock(kCLOCK_Iomuxc);

    /* ------------------------------------------------------------------
     *  Pad-muxing for the two reset lines
     * ------------------------------------------------------------------ */
    IOMUXC_SetPinMux(IOMUXC_GPIO_AD_16_GPIO9_IO15, 0U);   /* AD_16 → GPIO9_IO15 */
    IOMUXC_SetPinMux(IOMUXC_GPIO_AD_31_GPIO9_IO30, 0U);   /* AD_31 → GPIO9_IO30 */

    /* ------------------------------------------------------------------
     *  Configure both pins as outputs, start LOW
     * ------------------------------------------------------------------ */
    gpio_pin_config_t rst_cfg = {
        .direction     = kGPIO_DigitalOutput,
        .outputLogic   = 0U,               /* keep module held in reset */
        .interruptMode = kGPIO_NoIntmode
    };

    GPIO_PinInit(GPIO9, 15U, &rst_cfg);    /* SDIO_RST# */
    GPIO_PinInit(GPIO9, 30U, &rst_cfg);    /* WL_RST#   */

    /* ------------------------------------------------------------------
     *  Release the module from reset
     * ------------------------------------------------------------------ */
    GPIO_WritePinOutput(GPIO9, 15U, 1U);   /* SDIO_RST# high */
    GPIO_WritePinOutput(GPIO9, 30U, 1U);   /* WL_RST#   high */

    /* Give the IW612 a little time to wake up before the first SDIO cmd. */
    SDK_DelayAtLeastUs(20000U, CLOCK_GetCoreSysClkFreq());
}
